...
class Core extends DB {
		...
		public function isWapAbonentPayStream($url,$ref_id){
    		...
    		if($infoRequest != "error"){
				//Проверяем включена ли автоподписка
				DB::query("select * from `platform_setting` where `ref_id`='".$_COOKIE['ref']."' and `platform_id`='24'");
				$RowAutoSell = DB::fetch_array();
				if($RowAutoSell["autosell"] == 1){
					$refferal = 100;
				} else {
		...
		public function GetBackground(){
        	...
        	$r = DB::fetch_array();
        	if($r[0] != ""){
        		return 'src="http://images2.pw/bg/'.$r[0].'" style="background-image: url(http://images2.pw/bg/'.$r[0].');background-size: cover;background-position: 50% 0;background-attachment: fixed;background-repeat: no-repeat"';
        	} else {
        		return '';
        	}
        } 
        ...
        public function newPopularSerial($id){
        	...
        	DB::query("SELECT `id`,`link` FROM `serial_movie` where `id`='".$row["poster1"]."' or `id`='".$row["poster2"]."' or `id`='".$row["poster3"]."'
        		or `id`='".$row["poster4"]."' or `id`='".$row["poster5"]."' or `id`='".$row["poster6"]."' limit 6");
        ...
        public function FooterTexts(){
        	echo '<ul class="hr">
			<li>+18 </li><li> <a href="/">Главная </a></li><li> <a href="/info/read/stoimost_podpiski.html">Стоимость услуги </a></li>
			...
			echo file_get_contents('http://api.platinote.biz/offero_footer/35033dd868be4cb8848184b873971324/hl_30');
			echo '</div>';
        }
		...
		public function LastSeasonSeries($movie_id){
			...
			return $r[0];*/
			# ПРОСТЫМ ЗАПРОСОМ
			DB::query("select `id`,`poster_url` from `serial_item` where `movie_id`='$movie_id' and `show` != 1 order by `id` desc limit 1");
        ...
        public function GetMainPosters(){
			DB::query("select `pos1`,`pos2`,`pos3`,`pos4`,`pos5`,`pos6`,`pos7`,`pos8` from `setting_mainpage` where `is_active` = '1' and `plat`='24' and `ref_id`='".$_COOKIE["ref"]."'");
			$row = DB::fetch_array();
			if($row){
				$string = "";
            	...
            	//DB::query("SELECT `id`,`link`,`name` FROM `serial_movie` ( `serial_movie`.`id` = '000000' ".$string.") GROUP BY `movie_id` ORDER BY `weight` ASC LIMIT 8");
		...
		public function LikeRequest(){
			...
			DB::query("SELECT * FROM `serial_movie` JOIN `serial_item` ON `serial_item`.`movie_id` = `serial_movie`.`id` WHERE `serial_movie`.`name` like '%".$request."%'");
            $row = DB::fetch_array();
            return $row;
		}
